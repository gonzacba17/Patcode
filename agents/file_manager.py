...
class LoadedFile:
    ...
    size: int
    encoding: str = "utf-8"
    loaded_at: str = field(default_factory=lambda: __import__('datetime').datetime.now().isoformat())
...
class FileManager:
    ...
    def load_file(self, file_path: str) -> LoadedFile:
        ...
        if path.suffix.lower() not in self.ALLOWED_EXTENSIONS:
            raise PatCodeError(
                ...
                f"Extensiones válidas: {', '.join(sorted(self.ALLOWED_EXTENSIONS))}"
            )
    ...
    def get_stats(self) -> Dict[str, any]:
        ...
        return {
            ...
            'usage_percent': round((total_size / self.max_total_size) * 100, 2),
